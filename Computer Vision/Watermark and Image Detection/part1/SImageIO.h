...
// modified by Nawaz to handle non square images
 ...
 static SDoublePlane read_png_file(const char *file_name)
   {
     ...
     cout << "height : " << r.rows() << " : " << " width " << ": " << r.cols() << endl;

     for(int i=0; i<sz; i++){
        // cout << "r_ptr[i]: " << r_ptr[i]<< endl;
        //  cout << "g_ptr[i]: " << g_ptr[i]<< endl;
        //   cout << "b_ptr[i]: " << b_ptr[i]<< endl;
          gray_ptr[i] = r_ptr[i] * 0.3 + g_ptr[i] * 0.6 + b_ptr[i] * 0.1;
          // cout << "gray_ptr[i]: " << gray_ptr[i]<< endl;
          // cout << "&gray: " << &gray<< endl;
	...
	abort_("[read_png_file] Error during read_image");
      ...
      if(height == width){
         ...
         for(int i=0; i<height; i++)
   	{
         ...
         const int bytes_per_pixel = png_get_rowbytes(png_ptr, info_ptr) / width;
         assert(bit_depth == 8);
         _DTwoDimArray<unsigned char> img(height, width2*bytes_per_pixel);
         png_read_image(png_ptr, img.row_pointers());
         red_plane = SDoublePlane(height, width2);
         green_plane = SDoublePlane(height, width2);
         blue_plane = SDoublePlane(height, width2);
         for(int i=0; i<height; i++){
            const unsigned char *img_cp = img[i];
            double *r_ptr = red_plane[i], *g_ptr = green_plane[i], *b_ptr = blue_plane[i];
            for(int j=0, j2=0; j<width2; j++, j2+=bytes_per_pixel){
               if(j<width){
                  r_ptr[j] = img_cp[j2];
                  g_ptr[j] = img_cp[j2+1];
                  b_ptr[j] = img_cp[j2+2];
         ...
         const int bytes_per_pixel = png_get_rowbytes(png_ptr, info_ptr) / width;
         assert(bit_depth == 8);
         _DTwoDimArray<unsigned char> img(height2, width*bytes_per_pixel);
         png_read_image(png_ptr, img.row_pointers());
         red_plane = SDoublePlane(height2, width);
         green_plane = SDoublePlane(height2, width);
         blue_plane = SDoublePlane(height2, width);
         for(int i=0; i<height; i++){
            const unsigned char *img_cp = img[i];
            //cout << "red_plane[i]: "<<red_plane[i]<<":";
            double *r_ptr = red_plane[i], *g_ptr = green_plane[i], *b_ptr = blue_plane[i];
            for(int j=0, j2=0; j<width; j++, j2+=bytes_per_pixel){
                  r_ptr[j] = img_cp[j2];
                  g_ptr[j] = img_cp[j2+1];
