    ...
    for r in range(0,N):
        ...
        for c in range(0,N):
            if(board[r][c] == 1):#implies if 1 is at [1,1] do not re add onto it
                ...
                if(itemsOnBoard <= N-1 ):
                    # to remove adding of more than 3 items
                    # #if the item is present a row, exclude that row
                    if(itemsOnRow == 0):
                        #if the item not present in that row, then add item
                        if(count_on_col(board,c) == 0):
                            #if item not present in that column, then add item
